...
{
    public final HashMap<String, Integer> customData = new HashMap<>();
    ...
    public ItemActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit)
    {
        ...
        Integer c = this.customData.getOrDefault(uuid, 0);

        if (stack.getItem() == ModItems.RUBY_TRIAL_KEY)
        {
            if (c >= 0 && c < 4)
            {
                this.customData.put(uuid, c+1);
    ...
    public void writeNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registryLookup)
    {
        ...
        for (Map.Entry<String, Integer> entry: this.customData.entrySet())
        {
    ...
    public void readNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registryLookup)
    {
        ...
        for (int i = 0; i < nbtList.size(); i++)
        {
            ...
            this.customData.put(uuid, value);
        }
    }
